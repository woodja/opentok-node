...
var VgJwtGenerator = require('./VgJwtGenerator');
var OpenTok;
var key;
var keyPath;
var keyData;
var appId;
var vgAuth;
var vgJwtGeneratorInstance = new VgJwtGenerator();
...
OpenTok = function (apiKey, apiSecret, options) {
  var apiConfig;
  var clientConfig;
  var config;
  var jwt;
  var env = options.env;
  vgAuth = options.vgAuth;
  var claims;
  ...
  if (vgAuth) {
    appId = apiKey; // The first parameter is the VG app ID
    keyPath = apiSecret; // The first parameter is the path to private key file


    if (!_.isString(keyPath)) {
      throw new Error('Invalid arguments when initializing OpenTok: keyPath=' + keyPath);
    }
    keyData = fs.readFileSync(keyPath, 'utf8');
    claims = {
      application_id: appId
    };
    jwt = vgJwtGeneratorInstance.generate(keyData, claims);
  ...
  *     for a composed archive. This object has three properties: <code>type</code> and
  ...
  * to null.
  *
  * @param screenshareType {String} (Optional) The layout type to use when there is a screen-sharing
  ...
  */
   ...
   *     the broadcast. This object has three properties: <code>type</code> and
   *     <code>stylesheet</code>, and <code>screenshareType</code>, which are each strings.
   *     Valid values for the <code>layout</code> property are  <code>"bestFit"</code>
  ...
  */
   ...
   * connected to that session. This is a beta feature.
   *
...
/**
  ...
  *     or not (<code>false</code>, the default). This is a beta feature. See
...
OpenTok.prototype.generateToken = function (sessionId, opts) {
  ...
  if (vgAuth) {
    ...
    return vgJwtGeneratorInstance.generate(keyData, claims);
  }
